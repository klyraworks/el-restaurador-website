...
const S = {
  ...
  primary: { display: "inline-flex", alignItems: "center", gap: "6px", padding: "8px 16px", border: "none", borderRadius: "8px", background: "#10121A", fontSize: "13px", color: "#fff", cursor: "pointer", fontFamily: "inherit", fontWeight: 600, transition: "opacity 0.15s" },
  ghost:   { display: "inline-flex", alignItems: "center", gap: "6px", padding: "8px 14px", border: "1px solid #E4E4E7", borderRadius: "8px", background: "transparent", fontSize: "13px", color: "#71717A", cursor: "pointer", fontFamily: "inherit", transition: "background 0.15s, border-color 0.15s" },
  danger:  { display: "inline-flex", alignItems: "center", gap: "6px", padding: "8px 16px", border: "none", borderRadius: "8px", background: "#EF4444", fontSize: "13px", color: "#fff", cursor: "pointer", fontFamily: "inherit", fontWeight: 600, transition: "opacity 0.15s" },
...
function Select({ value, onChange, children, style }: { value: string; onChange: (e: React.ChangeEvent<HTMLSelectElement>) => void; children: React.ReactNode; style?: React.CSSProperties }) {
    ...
    <>
      ...
      <div style={{ padding: "32px 28px 60px" }}>
        ...
        <div style={{ display: "flex", gap: "8px", marginBottom: "16px", alignItems: "center" }}>
          ...
          <Select value={filtroEstado} onChange={e => { setFiltroEstado(e.target.value); setPage(1); }} style={{ width: "160px" }}>
            <option value="">Todos los estados</option>
            {ESTADOS.map(e => <option key={e} value={e}>{EST[e]?.label ?? e}</option>)}
          </Select>
          <Select value={filtroColor} onChange={e => { setFiltroColor(e.target.value); setPage(1); }} style={{ width: "160px" }}>
          ...
          {(filtroEstado || filtroColor) && (
            <button className="btn-ghost" onClick={() => { setFiltroEstado(""); setFiltroColor(""); setPage(1); }} style={{ ...S.ghost, padding: "7px 12px", fontSize: "12px" }}>
              <IconX /> Limpiar
            </button>
          )}
        </div>
              ...
              {loading ? (
                <tr>
                  <td colSpan={7} style={{ padding: "48px", textAlign: "center" }}>
                    <div style={{ display: "inline-flex", flexDirection: "column", alignItems: "center", gap: "10px" }}>
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#D4D4D8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ animation: "spin 1s linear infinite" }}>
                ...
                <tr>
                  <td colSpan={7} style={{ padding: "56px", textAlign: "center" }}>
                    <p style={{ fontSize: "13px", color: "#A1A1AA", marginBottom: "12px" }}>No hay servicios registrados</p>
                    <button onClick={openCreate} style={{ ...S.primary, fontSize: "12px", padding: "7px 14px" }} className="btn-primary">
                ...
                return (
                  ...
                  >
                    <td style={{ ...S.td, fontSize: "11px", fontFamily: "monospace", color: "#C4C4C8" }}>#{s.id}</td>
                    <td style={S.td}>
                      <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                        ...
                        <div>
                          ...
                          <span style={{ fontSize: "11px", color: "#A1A1AA", marginLeft: "5px" }}>{COLORES[s.tricimoto_color] ?? s.tricimoto_color}</span>
                        </div>
                      </div>
                    </td>
                    ...
                    <td style={S.td}>
                      ...
                      {Number(s.monto_pendiente) > 0 && (
                        <span style={{ display: "block", fontSize: "11px", color: "#92400E", fontFamily: "monospace" }}>
                    ...
                    <td style={S.td}>
                      <span style={{
                        ...
                        <span style={{ width: "5px", height: "5px", borderRadius: "50%", background: est.dot, flexShrink: 0 }} />
                        {est.label}
                      </span>
                    </td>
                    ...
                    <td style={{ ...S.td }}>
                      <div style={{ display: "flex", gap: "5px", opacity: isHovered ? 1 : 0.5, transition: "opacity 0.15s" }}>
                        <button onClick={() => openEdit(s)} className="row-action-btn row-action-edit">
                          <IconEdit /> Editar
                        </button>
                        <button onClick={() => { setSelected(s); setModal("delete"); }} className="row-action-btn row-action-delete">
                          <IconTrash /> Eliminar
                        </button>
                      </div>
                    </td>
      ...
      {(modal === "create" || modal === "edit") && (
        <div
          style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", backdropFilter: "blur(2px)" }}
          onClick={e => { if (e.target === e.currentTarget) setModal(null); }}
        >
          <div style={{ background: "#fff", borderRadius: "16px", width: "100%", maxWidth: "480px", boxShadow: "0 24px 64px rgba(0,0,0,0.18)", animation: "modalIn 0.18s ease" }}>
            <div style={{ padding: "20px 24px", borderBottom: "1px solid #E4E4E7", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <button onClick={() => setModal(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "#A1A1AA", display: "flex", padding: "2px" }}>
                <IconX />
              </button>
            </div>
            <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: "14px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                ...
                <div>
                  ...
                  <Select {...field("tricimoto_color")} onChange={e => setForm(p => ({ ...p, tricimoto_color: e.target.value }))}>
                    {Object.entries(COLORES).map(([key, label]) => (
                      <option key={key} value={key}>{label}</option>
                    ))}
                  </Select>
              ...
              <div>
                ...
                <Select value={form.mecanico_id} onChange={e => setForm(p => ({ ...p, mecanico_id: e.target.value }))}>
              ...
              <div>
                ...
                <Textarea {...field("descripcion")} rows={3} placeholder="Descripción del servicio..." onChange={e => setForm(p => ({ ...p, descripcion: e.target.value }))} />
              </div>
            </div>
            <div style={{ padding: "16px 24px", borderTop: "1px solid #E4E4E7", display: "flex", justifyContent: "flex-end", gap: "8px" }}>
              ...
              <button onClick={handleSave} disabled={saving} className="btn-primary" style={{ ...S.primary, opacity: saving ? 0.6 : 1 }}>
          ...
          <div style={{ background: "#fff", borderRadius: "16px", width: "100%", maxWidth: "380px", boxShadow: "0 24px 64px rgba(0,0,0,0.18)", animation: "modalIn 0.18s ease" }}>
            ...
            <div style={{ padding: "20px 24px", display: "flex", gap: "14px", alignItems: "flex-start" }}>
              ...
              <p style={{ fontSize: "13px", color: "#3F3F46", lineHeight: "1.65" }}>
                ¿Eliminar el servicio de la tricimoto <strong style={{ color: "#10121A" }}>{selected.tricimoto_num} · {COLORES[selected.tricimoto_color] ?? selected.tricimoto_color}</strong>? Esta acción no se puede deshacer.
              </p>
            </div>
            <div style={{ padding: "16px 24px", borderTop: "1px solid #E4E4E7", display: "flex", justifyContent: "flex-end", gap: "8px" }}>
              <button onClick={() => setModal(null)} className="btn-ghost" style={S.ghost}>Cancelar</button>
              <button onClick={handleDelete} disabled={saving} className="btn-danger" style={{ ...S.danger, opacity: saving ? 0.6 : 1 }}>
