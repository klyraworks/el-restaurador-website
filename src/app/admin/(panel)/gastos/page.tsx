...
function FocusSelect({ value, onChange, children }: { value: string; onChange: (e: React.ChangeEvent<HTMLSelectElement>) => void; children: React.ReactNode }) {
  ...
  return (
    <div style={{ position: "relative" }}>
      <select value={value} onChange={onChange} onFocus={() => setF(true)} onBlur={() => setF(false)} style={{ ...S.input, appearance: "none", paddingRight: "32px", borderColor: f ? "#10121A" : "#E4E4E7", boxShadow: f ? "0 0 0 3px rgba(16,18,26,0.08)" : "none", cursor: "pointer" }}>
        {children}
      </select>
...
function FocusTextarea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  const [f, setF] = useState(false);
  return <textarea {...props} onFocus={e => { setF(true); props.onFocus?.(e); }} onBlur={e => { setF(false); props.onBlur?.(e); }} style={{ ...S.input, resize: "vertical", borderColor: f ? "#10121A" : "#E4E4E7", boxShadow: f ? "0 0 0 3px rgba(16,18,26,0.08)" : "none", ...props.style }} />;
}
    ...
    <>
      ...
      <div style={{ padding: "32px 28px 60px" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "24px" }}>
          <div>
            ...
            <p style={{ fontSize: "13px", color: "#A1A1AA", marginTop: "3px" }}>{total > 0 ? `${total} registros` : "Gastos y adelantos del taller"}</p>
          </div>
          <button onClick={() => { setForm({ ...DEF }); setSelected(null); setModal("create"); }} className="btn-primary" style={S.primary}>
        ...
        <div style={{ display: "flex", gap: "8px", marginBottom: "16px", alignItems: "center" }}>
          ...
          <div style={{ width: "160px" }}>
            <FocusSelect value={filtroTipo} onChange={e => { setFiltroTipo(e.target.value); setPage(1); }}>
          ...
          {filtroTipo && (
            <button className="btn-ghost" onClick={() => { setFiltroTipo(""); setPage(1); }} style={{ ...S.ghost, padding: "7px 12px", fontSize: "12px" }}>
              <IconX /> Limpiar
            </button>
          )}
        </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ borderBottom: "1px solid #E4E4E7", background: "#FAFAFA" }}>
                {["#", "Tipo", "Monto / Descripción", "Registrado por", "Fecha", ""].map(h => <th key={h} style={S.th}>{h}</th>)}
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} style={{ padding: "56px", textAlign: "center" }}>
                  <p style={{ fontSize: "13px", color: "#A1A1AA", marginBottom: "12px" }}>Sin gastos registrados</p>
                  <button onClick={() => { setForm({ ...DEF }); setSelected(null); setModal("create"); }} className="btn-primary" style={{ ...S.primary, fontSize: "12px", padding: "7px 14px" }}>
                ...
                return (
                  <tr key={g.id} onMouseEnter={() => setHoveredRow(g.id)} onMouseLeave={() => setHoveredRow(null)}
                    style={{ borderBottom: i < rows.length - 1 ? "1px solid #F4F4F5" : "none", background: isHovered ? "#FAFAFA" : "#fff", transition: "background 0.12s" }}>
                    <td style={{ ...S.td, fontSize: "11px", fontFamily: "monospace", color: "#C4C4C8" }}>#{g.id}</td>
                    <td style={S.td}>
                      <span style={{ display: "inline-flex", alignItems: "center", gap: "5px", fontSize: "11px", fontWeight: 600, padding: "3px 9px", borderRadius: "20px", background: t.bg, color: t.color, border: `1px solid ${t.border}` }}>
                    ...
                    <td style={S.td}>
                      ...
                      {g.descripcion && <span style={{ display: "block", fontSize: "11px", color: "#A1A1AA", marginTop: "2px" }}>{g.descripcion}</span>}
                    </td>
                    ...
                    <td style={S.td}>
                      <div style={{ display: "flex", gap: "5px", opacity: isHovered ? 1 : 0.5, transition: "opacity 0.15s" }}>
                        <button onClick={() => openEdit(g)} className="ra-edit"><IconEdit /> Editar</button>
                        <button onClick={() => { setSelected(g); setModal("delete"); }} className="ra-del"><IconTrash /> Eliminar</button>
                      </div>
                    </td>
                  </tr>
                );
      ...
      {(modal === "create" || modal === "edit") && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", backdropFilter: "blur(2px)" }}
          ...
          <div style={{ background: "#fff", borderRadius: "16px", width: "100%", maxWidth: "400px", boxShadow: "0 24px 64px rgba(0,0,0,0.18)", animation: "modalIn 0.18s ease" }}>
            ...
            <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: "14px" }}>
              <div>
                ...
                <FocusSelect value={form.tipo} onChange={e => setForm(f => ({ ...f, tipo: e.target.value }))}>
                  <option value="gasto">Gasto</option>
                  <option value="adelanto">Adelanto</option>
                </FocusSelect>
              </div>
              ...
              <div>
                ...
                <FocusTextarea value={form.descripcion} onChange={e => setForm(f => ({ ...f, descripcion: e.target.value }))} rows={3} placeholder="Descripción opcional..." />
              </div>
            </div>
      ...
      {modal === "delete" && selected && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", backdropFilter: "blur(2px)" }}
          ...
          <div style={{ background: "#fff", borderRadius: "16px", width: "100%", maxWidth: "380px", boxShadow: "0 24px 64px rgba(0,0,0,0.18)", animation: "modalIn 0.18s ease" }}>
            <div style={{ padding: "20px 24px", borderBottom: "1px solid #E4E4E7", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <h2 style={{ fontSize: "15px", fontWeight: 700, color: "#10121A" }}>Eliminar {TIPO_STYLE[selected.tipo]?.label.toLowerCase()}</h2>
              <button onClick={() => setModal(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "#A1A1AA", display: "flex", padding: "2px" }}><IconX /></button>
            </div>
            <div style={{ padding: "20px 24px", display: "flex", gap: "14px", alignItems: "flex-start" }}>
              ...
              <p style={{ fontSize: "13px", color: "#3F3F46", lineHeight: "1.65" }}>
                ¿Eliminar este {TIPO_STYLE[selected.tipo]?.label.toLowerCase()} de <strong style={{ color: "#10121A" }}>${Number(selected.monto).toFixed(2)}</strong>? Esta acción no se puede deshacer.
              </p>
            </div>
            <div style={{ padding: "16px 24px", borderTop: "1px solid #E4E4E7", display: "flex", justifyContent: "flex-end", gap: "8px" }}>
              <button onClick={() => setModal(null)} className="btn-ghost" style={S.ghost}>Cancelar</button>
