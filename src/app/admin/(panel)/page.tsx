...
export default async function DashboardPage() {
  ...
  const th = { padding: "10px 16px", textAlign: "left" as const, fontSize: "10px", fontWeight: 700, textTransform: "uppercase" as const, letterSpacing: "0.07em", color: "#A1A1AA" };
  ...
  return (
    <div style={{ padding: "32px 28px 60px" }}>
      ...
      <div style={{ marginBottom: "28px", display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
        <div>
          <h1 style={{ fontSize: "22px", fontWeight: 800, color: "#10121A", letterSpacing: "-0.5px" }}>Dashboard</h1>
          <p style={{ fontSize: "13px", color: "#A1A1AA", marginTop: "3px" }}>Resumen general del taller</p>
        </div>
        <div style={{ textAlign: "right" }}>
          <p style={{ fontSize: "10px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "#A1A1AA", marginBottom: "3px" }}>Balance neto</p>
          <p style={{ fontSize: "20px", fontWeight: 800, fontFamily: "monospace", letterSpacing: "-0.5px", color: netBalance >= 0 ? "#166534" : "#991B1B" }}>
      ...
      <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", marginBottom: "28px" }}>
        {statCards.map(({ label, value, prefix }) => (
          <div key={label} style={{ background: "#fff", border: "1px solid #E4E4E7", borderRadius: "12px", padding: "16px 20px", flex: "1", minWidth: "140px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "10px" }}>
              <span style={{ color: "#A1A1AA" }}>{STAT_ICONS[label]}</span>
              <p style={{ fontSize: "10px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "#A1A1AA" }}>{label}</p>
            </div>
            ...
            {recientes.map((s, i) => {
              ...
              return (
                <tr key={s.id} style={{ borderBottom: i < recientes.length - 1 ? "1px solid #F4F4F5" : "none" }}>
                  <td style={{ ...td, fontSize: "11px", fontFamily: "monospace", color: "#C4C4C8" }}>#{s.id}</td>
                  <td style={td}>
                  ...
                  <td style={{ ...td, maxWidth: "220px" }}>
                    {s.descripcion
                      ? <span style={{ display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: "13px", color: "#3F3F46" }}>{s.descripcion}</span>
                  ...
                  <td style={{ ...td, fontSize: "11px", fontFamily: "monospace", color: "#A1A1AA" }}>{fecha}</td>
                </tr>
              );
            })}
          </tbody>
