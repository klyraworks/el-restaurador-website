...
export async function GET(req: NextRequest) {
  ...
  let i = 1;
  if (estado) { conditions.push(`s.estado = $${i++}`); params.push(estado); }
  if (color)  { conditions.push(`s.tricimoto_color = $${i++}`); params.push(color); }

  const where = conditions.join(" AND ");

  const rows = await query(`
    SELECT
      s.id, s.tricimoto_num, s.tricimoto_color, s.descripcion,
      s.monto_total, s.monto_pendiente, s.estado, s.created_at,
      u.nombre AS mecanico, u.id AS mecanico_id,
  ...
  const [{ total }] = await query<{ total: string }>(
    `SELECT COUNT(*) AS total FROM servicios s WHERE ${where}`, params
  );
...
export async function POST(req: NextRequest) {
  ...
  const row = await queryOne<{ id: number }>(`
    INSERT INTO servicios (tricimoto_num, tricimoto_color, monto_total, monto_pendiente, descripcion, mecanico_id, registrado_por)
    VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id
  `, [tricimoto_num, tricimoto_color, monto_total, monto_pendiente, descripcion ?? null, mecanico_id, session.user.id]);
...
export async function PATCH(req: NextRequest) {
  ...
  await query(`
    UPDATE servicios SET
      tricimoto_num=$1, tricimoto_color=$2, monto_total=$3,
      monto_pendiente=$4, descripcion=$5, mecanico_id=$6, estado=$7
...
export async function DELETE(req: NextRequest) {
  ...
  await query(`UPDATE servicios SET deleted_at=NOW(), is_active=false WHERE id=$1`, [id]);
